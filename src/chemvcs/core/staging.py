...
class StagingManager:
    ...
    def __init__(self, workspace_root: Path, object_store: ObjectStore):
        ...
        if not self.chemvcs_dir.exists():
            raise StagingError(f"Not a ChemVCS repository (no .chemvcs/ found in {workspace_root})")
    ...
    def _resolve_path(self, path: Path) -> Path:
        ...
        abs_path = path if path.is_absolute() else (self.workspace_root / path).resolve()
    ...
    def _add_directory(
        ...
        # Collect all files first to batch the add operation (avoids O(n) index writes)
        file_paths: list[Path] = []
        try:
            for item in dir_path.rglob("*"):
                if item.is_file() and not self._is_chemvcs_path(item):
        ...
        if not file_paths:
            return {"added": [], "updated": [], "ignored": [], "errors": []}

        return self.add(file_paths, force=force)
