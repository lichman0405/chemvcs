        ...
        **kwargs: Any,
        ...
        for validator in sorted_validators:
            ...
            try:
                ...
                console.print(f"[red]Validator '{validator.name}' failed: {e}[/red]")
    ...
    def _print_validation_result(self, validator_name: str, result: ValidationResult) -> None:
        ...
        if result.passed:
            if result.has_warnings:
                console.print(f"  [yellow]⚠[/yellow]  {validator_name}: {result.message}")
                for warning in result.warnings:
                    console.print(f"      [dim]{warning}[/dim]")
            else:
                console.print(f"  [green]✓[/green]  [dim]{validator_name}: passed[/dim]")
        else:
            console.print(f"  [red]✗[/red]  {validator_name}: {result.message}")
    ...
    def load_config(self, chemvcs_dir: Path) -> None:
        ...
        if config_path.exists():
            try:
                ...
                self._config = {}
        self._chemvcs_dir: Path | None = chemvcs_dir

    def save_config(self) -> None:
        """Persist current configuration to .chemvcs/plugins.json."""
        chemvcs_dir: Path | None = getattr(self, "_chemvcs_dir", None)
        if chemvcs_dir is None:
            return
        config_path = chemvcs_dir / _PLUGIN_CONFIG_FILE
        # Atomic write via temp file
        fd, tmp_path = tempfile.mkstemp(
            dir=chemvcs_dir, prefix=".tmp_plugins_", suffix=".json"
        )
        try:
